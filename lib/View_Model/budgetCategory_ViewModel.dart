...
class BudgetCategoryViewModel extends ChangeNotifier {
  ...
  Future<void> getSubCategories() async {
    try {
      Iterable<Category> subcategorytemp =
      ...
      if (subcategorytemp.isNotEmpty) {
        ...
        subCategoriess = await BudgetStorageController.getSubCategories(
            subcategorytemp.first.categoryid);
  ...
  addCategory() async {
    Category category = Category(
        type: "user", label: textFormValidator.text.trim(), categoryid: "");
  ...
  addBudgetAmount() async {
    try {
      ...
      if (categorytemp.isNotEmpty && selectedBudgetId != null) {
        ...
        isBudgetAdding = true;
        notifyListeners();
        bool status = await BudgetStorageController.addBudgetAmount(
            budgetAmount, selectedBudgetId!);
        if (status) {
    ...
    try {
      ...
      if (subcategorytemp.isNotEmpty) {
        SubCategory subCategory = SubCategory(
            subcategoryid: "",
            label: textFormValidator.text.trim(),
            categoryid: subcategorytemp.first.categoryid,
  ...
  deleteCategory(int index) async {
    try {
      await BudgetStorageController.deleteCategory(
          categoriess[index].categoryid);
